...
const TeamHero = () => {
  ...
  useEffect(() => {
    ...
    // Auto-rotate team highlights
    const interval = setInterval(() => {
      setCurrentHighlight((prev) => (prev + 1) % teamHighlights.length);
    }, 3000);
      ...
      <div className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-black to-transparent flex justify-center items-end">
        <div className="flex space-x-8 mb-4">
          {[1, 2, 3, 4].map((item) => (
            <div
              key={item}
              className="w-12 h-16 bg-amber-500/10 rounded-t-lg border border-amber-500/20"
            ></div>
          ))}
        </div>
      </div>
